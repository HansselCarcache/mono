...
namespace SistemaEmpleadosEyS.Datos
{
    public class DT_tbl_Cargo
    {
        ...
        {
            ListStore empleado_datos = new ListStore(typeof(int), typeof(string), typeof(int));

            sb.Clear();
            sb.Append("Use ControlBD;");
            sb.Append("SELECT * FROM ControlBD.Cargo;");
            try
            {
                con.AbrirConexion();
                ...
                while (idr.Read())
                {
                    empleado_datos.AppendValues(idr[0], idr[1], idr[2]);
                }
        ...
        /*public bool guardarDepartamento(tbl_cargo car)
        {
            bool guardado = false;
            int x = 0;
            sb.Clear();
            sb.Append("INSERT INTO ControlBD.Cargo");
